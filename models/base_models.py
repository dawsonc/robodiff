...
class BaseModel(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            optimizer is None
            or isinstance(optimizer, torch.optim.Optimizer)
            or isinstance(optimizer, dict)
        ), "Optimizer must be either None or a torch.optim.Optimizer or a dictionary which will be auto-expanded into an optimizer"
        self._genome = genome
    ...
    def get_summary_string(self, grads=True, truncate=0, name="Base Model"):
        return pprint.pformat(
            self.get_genome(grads=grads, truncate=truncate, force_include_genome=True)
        )

    def validateOptimizerGenome(self):
        optimizerGenomeType = type(self._optimizerGenome)
        assert optimizerGenomeType in [
            type(None),
            list,
            torch.Tensor,
        ], "optimizerGenome must be of type NoneType, list, or torch.Tensor"

        if optimizerGenomeType == list:
            for element in self._optimizerGenome:
                assert isinstance(
                    element, torch.Tensor
                ), "if optimizerGenome is a list, it must only contain objects of type torch.Tensor"
        ...
        for k, v in self._genome.items():
            if not isinstance(v, torch.Tensor):
                ...
                # else: try to decode as a python list or an numpy array.
                    ...
                    arrays_in_genome[k] = np.array(
                        v, dtype=np.float32
                    ).tolist()  # set string NaN and Inf to float values. Torch.tensor does not like strings.

        # some error checking
        if len(arrays_in_genome) != 0 and self._optimizerGenome is not None:
            raise ValueError(
                "optimizerGenome can not be set if genome contains any elements that are not torch.Tensor objects"
            )

        # convert python lists to torch objects.
        for k, v in arrays_in_genome.items():
            try:
                self._genome[k] = torch.tensor(
                    v, device=self.device, requires_grad=True
                )
            except:
    ...
    def refresh_optimizer(self):
        if isinstance(self._optimizer, dict):
            ...
            optimizerGenome = (
                self._optimizerGenome
                if self._optimizerGenome is not None
                else list(self._genome.values())
            )
            optimizerLR = self._optimizer["lr"]
            if optimizerName == "Adam":
                self._optimizer = torch.optim.Adam(optimizerGenome, lr=optimizerLR)
            elif optimizerName == "MALA":
                self._optimizer = MALAOptimizer(optimizerGenome, lr=optimizerLR)
            else:
                raise NotImplementedError(
                    "Base Model does not support generating an optimizer of type other than Adam"
                )

            # Add fine-tuning optimizer
            self._fine_tune_optimizer = MALAOptimizer(optimizerGenome, lr=1e-5)
    ...
    def validate_grads(self, key, value):
        if isinstance(value, np.ndarray):
            ...
            raise ValueError(
                f"{key} must be of type numpy.ndarray or torch.tensor. It is: {type(value)}"
            )
    ...
    """
    Perform a fine-tuning optimization step if applicable.
    """

    def fine_tune_step(self, closure=None):
        if self._fine_tune_optimizer is not None:
            self._fine_tune_optimizer.step(closure)
    ...
    def get_genome(
        self, grads=True, truncate=0, force_include_genome=False, sanitize=False
    ):
        dict_to_return = dict()
        if (
            self._optimizer is None
            and not self._always_include_genome
            and not force_include_genome
        ):
        ...
        for k, v in self._genome.items():
            if isinstance(v, torch.Tensor):
                ...
                if v.grad is not None and grads:
                    dict_to_return[f"{k}_grad"] = (
                        v.grad.detach().cpu().numpy().tolist()[:truncate]
                    )
    ...
    def export_genome_b64(self, grads=True, sim_grads=True):
        ...
        if sim_grads:
            items_to_save += [
                (f"grad_from_sim_{k}", v) for k, v in self._grads_dict.items()
            ]
        for k, v in items_to_save:
            ...
            dict_to_return[k] = base64.urlsafe_b64encode(pickle.dumps(val)).decode(
                "utf-8"
            )
            if curr_grads is not None:
                dict_to_return[f"{k}_grad"] = base64.urlsafe_b64encode(
                    pickle.dumps(curr_grads)
                ).decode("utf-8")
...
class BaseMorphModel(BaseModel):
    ...
    def backward(self, **kwargs):
        ...
        assert (
            "mass" in kwargs
        ), "BaseMorphModel must be passed 'mass' as a keyword argument to the backward method"
...
class BaseActuationModel(BaseModel):
    ...
    def generate(self):
        ...
        return dict(
            (k, v.detach().cpu().numpy()) for k, v in self.output_tensors.items()
        )
    ...
    def backward(self, **kwargs):
        ...
        for k in keys_of_interest:
            assert (
                k in kwargs
            ), f"BaseActuationModel must be passed '{k}' as a keyword argument to the backward method"

        output_tensors_filtered = dict(
            (k, v) for k, v in self.output_tensors.items() if k in keys_of_interest
        )
        cat_output_tensors = torch.cat(
            [v for k, v in sorted(output_tensors_filtered.items())]
        )
        cat_grads = torch.cat(
            [self.validate_grads(k, v) for k, v in sorted(kwargs.items())]
        )
        # IPython.embed()
