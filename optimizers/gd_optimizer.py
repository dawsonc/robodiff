...
import torch
from models.utils import b64_encode, sanitize_num
...
class GDOptimizer(Optimizer):
    ...
    def _opt_step(self, idx, disable_optimization, max_steps):
        # save the genome corresponding to the loss that we are about to get...
        self._simModel.zero_grad()

        results_dict = self._simulate(
            title=self._simModel.get_title(),
        ...
        simModelExportInfo = self._simModel.get_export_info(
            epoch_idx=idx, loss=sanitize_num(results_dict["loss"]), x_avg=x_avg_b64
        )
        ...
        if self._results_handler:
            ...
            if didv.args.resim:
                ...
                resim_results = self._simulate(
                    title=self._simModel.get_title(), enable_gui=False, epoch_idx=idx
                )
        ...
        if not disable_optimization:
            if self.use_closure:
                ...
                if idx <= max_steps // 2:
                    ...
                    self._simModel.fine_tune_step(closure)
            else:
                if idx <= max_steps // 2:
                    self._simModel.step()
                else:
                    self._simModel.fine_tune_step()
    ...
    def optimize(self, disable_optimization):
        ...
        for idx in tq:
            ...
            msg = self._opt_step(idx, disable_optimization, self._epochs)
