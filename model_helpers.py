...
def get_mala_optimizer(lr=0.01):
    return {"name": "Adam", "lr": lr}


def get_morph_rand_direct_circle_fixed_size(
    hole_count=1, optimize=True, holeSize=0.2, target_hole_area=None, max_hole_size=None
):
    dict_to_return = get_morph_rand_direct_circle(
...
def get_morph_rand_direct_circle(
    hole_count=1, optimize=True, holeSize=0.2, target_hole_area=None, max_hole_size=None
):
...
def get_morph_rand_direct_circle_multi_sized_holes(
    hole_count=1,
    optimize=True,
    hole_fraction=0.6,
    ...
    mean_hole_size = compute_mean_hole_size(
        hole_fraction, robot_aspect_ratio, hole_count
    )
    hole_x = np.random.random(hole_count).tolist()
    hole_y = (np.random.random(hole_count) * robot_aspect_ratio).tolist()
    hole_sizes = (
        [mean_hole_size]
        if shared_hole_sizes
        else np.random.normal(
            mean_hole_size, mean_hole_size**2, size=(hole_count)
        ).tolist()
    )
    if power_law_hole_sizes:
        sizes = 0.25 / np.power(2, (np.ceil(np.log2(np.arange(1, hole_count + 1)))))
        total_area = (
            np.power(sizes, 2).sum() * np.pi / (robot_aspect_ratio * hole_fraction)
        )
...
def get_morph_direct_particle(
    mode="mass", mass=[1.0] * 64 * 44, x=[[0.0, 0.0]] * 64 * 44
):
    return {
        "name": "DirectParticleMorphModel",
        "mode": mode,
        "genome": {"mass": mass, "x": x},
...
def get_morph_default_particle():
    return {"name": "DefaultParticleMorphModel"}
...
def get_act_rand_direct_circle_sine_only(
    hole_count=1, optimize=True, actuation_omega=40.0
):
...
def get_act_rand_direct_circle(
    hole_count=1, optimize=True, actuation_omega=40.0, robot_aspect_ratio=0.7
):
    return get_act_direct_circle(
        hole_x_sine=np.random.random(hole_count).tolist(),
